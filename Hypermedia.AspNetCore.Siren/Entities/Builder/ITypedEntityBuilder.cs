namespace Hypermedia.AspNetCore.Siren.Entities.Builder
{
    ...
    public interface ITypedEntityBuilder<T> where T : class
    {
        ...
        ITypedEntityBuilder<T> WithAction<TBody>(string name, Expression<Action<T>> endpointCapture, Action<IActionBuilder<TBody>> configureAction) where TBody : class;

        ITypedEntityBuilder<T> WithEntity(Action<IEntityBuilder> configure);
        ITypedEntityBuilder<T> WithEntity(Expression<Action<T>> collectEntity, params string[] classes);
        ...
        ITypedEntityBuilder<T> WithLink(string name, Expression<Action<T>> captureLink, params string[] rel);
